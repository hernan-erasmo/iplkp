...
class IplkpCache():
    ...
    def _init_from_file(self):
        try:
            ...
            print(f"Unable to read from cache data source ({e}). Starting with empty cache.")
        else:
            return data
        ...
        try:
            ...
            raise IplkpCacheException(f"Unable to create cache file ({e})")
        else:
            return file_structure

    def get_rdap(self, ip):
        return ip in self._data_source[RDAP_LOOKUP_TASK_NAME]

    def get_geo_ip(self, ip):
        return ip in self._data_source[GEO_IP_LOOKUP_TASK_NAME]
    ...
    def find_all(self, addresses, just_rdap=False, just_geo=False):
        """Input:
            ...
            - it's both an rdap and geo_ip query, and the cache contains info about both for the given ip
        """
        ...
        for ip in addresses:
            if query_all:
                if all([self.get_rdap(ip), self.get_geo_ip(ip)]):
                    cached_results[RDAP_LOOKUP_TASK_NAME][ip] = self._data_source[RDAP_LOOKUP_TASK_NAME][ip]
                    cached_results[GEO_IP_LOOKUP_TASK_NAME][ip] = self._data_source[GEO_IP_LOOKUP_TASK_NAME][ip]
                else:
        ...
        print(f"returning cached results for {min([len(cached_results[k]) for k in cached_results.keys()])} IPs, missing {len(missing_IPs)}")
